...
func cmd() *cobra.Command {
	...
	c := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				log.Fatalf("Error loading SAML cert/key [%+v]", err)
			}
			...
			idpMetadataURL, err := url.Parse(issuerURL + "/metadata")
			if err != nil {
				log.Fatalf("Error creating parsing SAML metadata URL [%+v]", err)
			}

			ssp, err := samlsp.New(samlsp.Options{
				URL:            *listenURL,
				Key:            keyPair.PrivateKey.(*rsa.PrivateKey),
				Certificate:    keyPair.Leaf,
				IDPMetadataURL: idpMetadataURL,
				// TODO - we want to force SAML flow, but need cookies for other
				// session stuff. how do we fix?
				//CookieMaxAge:   1 * time.Second, // avoid sessions
			})
			if err != nil {
				log.Fatalf("Error creating SAML service provider [%+v]", err)
			}

			http.Handle("/saml/", ssp)

			http.Handle("/samlsecure", ssp.RequireAccount(http.HandlerFunc(a.handleSamlSecured)))
	...
	c.Flags().StringVar(&issuerURL, "issuer", "http://localhost:5556", "URL of the OpenID Connect issuer.")
	...
	c.Flags().StringVar(&samlKey, "saml-key", "testdata/samlsp.key", "SAML SP Key")
	c.Flags().StringVar(&samlCert, "saml-cert", "testdata/samlsp.cert", "SAML SP Cert")
...
func (a *app) handleSamlSecured(w http.ResponseWriter, r *http.Request) {
