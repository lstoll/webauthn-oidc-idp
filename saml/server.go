...
func (s *Server) MountRoutes(mux *chi.Mux) {
	...
	mux.Handle("/metadata", s.IDPServer)
	mux.Handle("/sso", s.IDPServer)
	// mux.Handle("/login", s.IDPServer)

	// // Our bits - maybe not needed, maybe /sso does all?
	// mux.HandleFunc("/samllogin", s.Connector.loginHandler)
	// mux.HandleFunc("/samlfinal", s.Connector.Finalize)
}
